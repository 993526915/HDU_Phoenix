...
#include<iostream>
//#include"tool.h"
//cv_tool image_buffer("cap0");
cv::Mat src;
using namespace boost;

Mycamera::Mycamera(/* args */): image(480,640,CV_8UC3,(0,0,0))
{
        nRet = MV_OK;//flag
        stImageInfo = {0};
...
bool Mycamera::rgbtocv()
{
        ...
        //image_buffer.ImageCallback(src);
        return true;
       /*  //way 2.
        imag =  */
}
cv::Mat Mycamera::getiamge(void)       
{
        ...
        return image;
        /* cv::Mat returnimage;
        int id = image_buffer.NextImage(returnimage);
        image_buffer.ReadComplete(id);
        if(id!=-1)
        {
                cv::imshow("retu",returnimage);
                return returnimage;
        } */
}
